...
const (
	binRootName          = "poller_exporter"
	dockerImageName      = "wrouesnel/poller_exporter"
...
var toolsSrcDir = normalizePath(path.Join(toolDir, "src"))
var toolsBinDir = normalizePath(path.Join(toolDir, "bin"))
var toolsVendorDir = normalizePath(path.Join(toolDir, "vendor"))

var outputDirs = []string{binDir, releaseDir, toolsGoPath, toolsBinDir,
	toolsVendorDir, assetsGenerated, coverageDir}

var toolsEnv = map[string]string{"GOPATH": mustStr(filepath.EvalSymlinks(toolsGoPath))}
...
func getMetalinters() []string {
	...
	linterRx := regexp.MustCompile(`[\ ]+\w+:\s*\((.+)\)`)
...
func UpdateTools() error {
	// Always ensure we sanitize pkg after this
	defer sh.Rm(path.Join(toolsGoPath, "pkg"))

	if _, err := exec.LookPath("dep"); err != nil {
		// Ensure govendor is up to date without doing anything
		govendorPkg := "github.com/golang/dep"
	...
	// Ensure tools src link is not present, but if we were interrupted, don't clean up.
	if _, err := os.Readlink(toolsSrcDir); err == nil {
		if rerr := sh.Rm(toolsSrcDir); rerr != nil {
			return rerr
		}
		// Make it a real directory for the duration of the update
		if merr := os.MkdirAll(toolsSrcDir, os.FileMode(0755)); merr != nil {
			return merr
		}
	}

	// Make a tools dir under src/ and run the actual update from there
	toolsDepProject := path.Join(toolsSrcDir, "tools")
	if err := os.MkdirAll(toolsDepProject, os.FileMode(0777)); err != nil {
		return err
	}

	// Move the vendor dir so dep can update it if it exists
	os.Rename(toolsVendorDir, path.Join(toolsDepProject, "vendor"))

	// Copy dep files if they exist (i.e. ignore return codes
	CopyFile(path.Join(toolDir, "Gopkg.lock"), path.Join(toolsDepProject, "Gopkg.lock"))
	CopyFile(path.Join(toolDir, "Gopkg.toml"), path.Join(toolsDepProject, "Gopkg.toml"))

	// Set current directory to the tools dep project
	previousPwd, wderr := os.Getwd()
	if wderr != nil {
		return wderr
	}
	if err := os.Chdir(toolsDepProject); err != nil {
		return err
	}

	// Drop a main.go into the tools project
	toolsMain := path.Join(toolsDepProject, "main.go")
	if err := ioutil.WriteFile(toolsMain, []byte("package main\n"), os.FileMode(0600)); err != nil {
		return err
	}

	// Unconditionally run dep init
	sh.RunWith(toolsEnv, "dep", "init")

	// dep fetch tools
	toolsList := append(getCoreTools(), getMetalinters()...)
	depToml, err := toml.LoadFile("Gopkg.toml")
	if err != nil {
		return err
	}
	depToml.Set("required", toolsList)
	depTomlOut, err := os.OpenFile("Gopkg.toml", os.O_CREATE|os.O_WRONLY|os.O_TRUNC, os.FileMode(0600))
	if err != nil {
		return err
	}
	depToml.WriteTo(depTomlOut)
	depTomlOut.Close()

	if _, err := os.Stat("Gopkg.lock"); os.IsNotExist(err) {
		if err := sh.RunWith(toolsEnv, "dep", "ensure", "-v"); err != nil {
			return err
		}
	} else {
		if err := sh.RunWith(toolsEnv, "dep", "ensure", "-update", "-v"); err != nil {
			return err
		}
	}
	...
	// Copy dep files back
	CopyFile(path.Join(toolsDepProject, "Gopkg.lock"), path.Join(toolDir, "Gopkg.lock"))
	CopyFile(path.Join(toolsDepProject, "Gopkg.toml"), path.Join(toolDir, "Gopkg.toml"))

	// Check nothing's in the way
	sh.Rm(toolsVendorDir)

	// Move the vendor dir back
	if err := os.Rename(path.Join(toolsDepProject, "vendor"), toolsVendorDir); err != nil {
		return err
	}

	// Restore the vendor link
	if err := ensureVendorSrcLink(); err != nil {
		return err
	}
...
//			mustStr(filepath.Rel(curDir, p.PlatformBin(binRootName)))),
//		"-t", containerName, ".")
//}
...
func Autogen() error {
	...
	splitPreCommit := strings.Split(string(data), "\n")

	preCommitLine := "go run mage.go style || exit $?"

	if strings.TrimRight(splitPreCommit[0], " \t") != "#!/bin/bash" {
		fmt.Println("Don't know how to update your pre-commit script.")
		fmt.Println("Maybe try editing it manually to run the follow shell (or equivalent):")
		fmt.Println(preCommitLine)
		return errors.New("unknown pre-commit script format")
	}

	// Check if the pre-commit hook was already put in somewhere.
	needsHook := true
	firstNonComment := -1
	for idx, line := range splitPreCommit {
		// Discard comment only lines
		if found, _ := regexp.MatchString("\\s*#", line); found {
			continue
		}
		...
		if strings.Trim(line, " \t") == preCommitLine {
			needsHook = false
			break
		}
	}
	...
	if needsHook {
		if firstNonComment == -1 {
			firstNonComment = len(preCommitLine)
		}
		// Slice the hook in at the top of the script.
		updatedScript := append(splitPreCommit[:firstNonComment], preCommitLine)
		updatedScript = append(updatedScript, splitPreCommit[firstNonComment:]...)

		return ioutil.WriteFile(".git/hooks/pre-commit", []byte(strings.Join(updatedScript, "\n")),
			os.FileMode(0755))
	} else {
