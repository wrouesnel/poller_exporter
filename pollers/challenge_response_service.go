...
func (s *ChallengeResponseService) Status() Status {
	if s.Poller.Status() == PollStatusFailed || s.Poller.Status() == PollStatusUnknown {
...
func (this *ChallengeResponseService) Poll() {
	...
	if conn == nil {
		...
		this.serviceChallengeable = PollStatusUnknown
	...
	if this.isWriter() {
		...
		if this.isReader() {
			if this.serviceChallengeable == PollStatusSuccess {
				...
				this.serviceResponsive = PollStatusFailed
				this.serviceResponseTime = 0
	...
	this.log().Debug("Finished challenge_response poll.")
}
...
func (s *ChallengeResponseService) Challenge(conn io.Writer) Status {
	...
	if err != nil {
		s.log().Info("Connection error doing ChallengeResponse check", zap.Error(err))
	...
	for {
		...
		if s.ResponseRegex != nil {
			if s.ResponseRegex.Match(allBytes) {
				...
				s.log().Debug("Matched regex", zap.Uint64("bytes_till_match", nTotalBytes))
				break
			}
		} else {
			if bytes.HasPrefix(allBytes, []byte(*s.ResponseLiteral)) {
				...
				s.log().Debug("Matched byte literal", zap.Uint64("bytes_till_match", nTotalBytes))
				break
			}
		}
		...
		if nTotalBytes >= s.MaxBytes {
			s.log().Info("Maximum read bytes exceeded during check: read",
				zap.Uint64("bytes_read_without_match", nTotalBytes), zap.Uint64("max_bytes_till_fail", s.MaxBytes))
			break
		}
	}
